	...
	$.fn.fileUploader = function(config){
		...
		*/
		function uploadChange($this) {
			...
			var result = config.onFileChange($this, $form);
			if(result == -1){
				$form.find(':file').val('');
				return false;
			}
			...
			$(pxUploadForm).append(
				...
				.append(' \
					<ul class="actions ui-helper-clearfix"> \
						<li title="上传" class="upload ui-state-default ui-corner-all"> \
							<span class="ui-icon ui-icon-circle-triangle-e"></span> \
						</li> \
						<li title="取消" class="delete ui-state-default ui-corner-all"> \
		...
		*/
		function iframeUpload($form) {
			...
			var index = parseInt($form.attr("id").substring(8));
			
			var id = pxWidget + ' #' + $form.attr('id');
			$("#pxupload"+(index)+"_frame").load(function(){
		...
		*/
		function beforeEachUpload($form) {
			...
			$uploadData = $form.parent();
			$uploadData.find('.status').text('正在上传...');
...
//				$uploadData.children('.status').html( $(data).find('#message').text() );
				$uploadData.children('.status').html( data.responseStatus.message );
...
//					$uploadData.children('.status').html( $(data).find('#message').text() );
			...
			$('.delete', pxUploadForm).trigger("click");
		}
		...
		*/
		...
		/*	$(id+'_text').fadeOut('4000',function(){
				$(id+'_frame').remove();
				$(this).remove();
